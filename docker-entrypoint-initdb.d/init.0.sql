...
create schema mission_control_utils;

-- By setting the `search_path`, whenever we create something in the default
-- namespace it is actually created in the `mission_control` schema.
--
-- For example, this lets us write `create table person …` instead of
-- `create table mission_control.person …`.
set search_path = mission_control, mission_control_utils, public;
...
feature_flag_activated boolean, 
feature_flag_name varchar(100), 
feature_flag_description varchar(100)
);

comment on table feature_flags is 'A feature flag of the JB Website.';
comment on column feature_flags.id is 'The primary key for the Feature Flag.';
comment on column feature_flags.feature_flag_name is 'The Feature Flag name.';
comment on column feature_flags.feature_flag_description is 'The Feature Flag description.';
...
create table mission_control_utils.person_account (
...
--
-- Note that we also create them in `mission_control_utils` as we want them to be
...
create function mission_control_utils.set_created_at() returns trigger as $$
begin
...
create function mission_control_utils.set_updated_at() returns trigger as $$
begin
...
INSERT INTO feature_flags(id, feature_flag_name, feature_flag_description, feature_flag_activated) VALUES 
  (1, 'PhoneNumberValidation', 'Phone Number Validation', TRUE),
  (2, 'AddressValidation', 'Address Validation', FALSE),
  (3, 'BadPunHumour', 'Allow bad puns', FALSE);
...
grant select on person, post, feature_flags to public;
